...
// --- 文件持久化路径宏定义 ---
#define WINDOW_FILE "windows.txt"
#define DISH_FILE "dishes.txt"
...
int gIsSorted = 0;                   // 【新增】0表示默认顺序，1表示已排序
...
static Button gBtnClearSearch = { 750, 85, 870, 125, "恢复默认" };
...
void saveWindowsToFile(void) {
	FILE* fp = fopen(WINDOW_FILE, "w"); // "w" 文本写入模式
	if (!fp) return;
	WindowNode* curr = gWindowHead;
	while (curr) {
		fprintf(fp, "%s|%s\n", curr->data.windowId, curr->data.windowName);
		curr = curr->next;
	}
	fclose(fp);
}
void loadWindowsFromFile(void) {
	FILE* fp = fopen(WINDOW_FILE, "r"); // "r" 文本读取模式
	if (!fp) return;
	Window item;
	// %[^\n] 表示读取直到换行符，完美兼容带空格的窗口名
	while (fscanf(fp, "%[^|]|%[^\n]\n", item.windowId, item.windowName) == 2) {
...
void saveDishesToFile(void) {
	FILE* fp = fopen(DISH_FILE, "w");
	if (!fp) return;
	DishNode* curr = gDishHead;
	while (curr) {
		fprintf(fp, "%s|%s|%s|%s|%.2f|%d|%.2f|%d|%d\n",
			curr->data.dishId, curr->data.windowId, curr->data.name,
			curr->data.category, curr->data.price, curr->data.stock,
			curr->data.avgScore, curr->data.scoreCount, curr->data.sales);
		curr = curr->next;
	}
	fclose(fp);
}
void loadDishesFromFile(void) {
	FILE* fp = fopen(DISH_FILE, "r");
	if (!fp) return;
	Dish item;
	// 使用 %[^|] 确保带空格的菜品名称和分类也能被完整读取
	while (fscanf(fp, "%[^|]|%[^|]|%[^|]|%[^|]|%lf|%d|%lf|%d|%d\n",
		item.dishId, item.windowId, item.name, item.category,
		&item.price, &item.stock, &item.avgScore, &item.scoreCount, &item.sales) == 9) {
...
void saveUsersToFile(void) {
	...
	while (curr) {
		fprintf(fp, "%s|%s|%d\n", curr->data.username, curr->data.password, curr->data.role);
		curr = curr->next;
	}
	fclose(fp);
}
void loadUsersFromFile(void) {
	...
	if (!fp) return;
	User item;
	while (fscanf(fp, "%[^|]|%[^|]|%d\n", item.username, item.password, &item.role) == 3) {
...
void saveOrdersToFile(void) {
	...
	while (curr) {
		fprintf(fp, "%s|%s|%s|%s|%d|%.2f|%d|%s\n",
			curr->data.orderId, curr->data.username, curr->data.dishId,
			curr->data.dishName, curr->data.quantity, curr->data.totalPrice,
			curr->data.status, curr->data.orderTime);
		curr = curr->next;
	}
	fclose(fp);
}
void loadOrdersFromFile(void) {
	...
	while (fscanf(fp, "%[^|]|%[^|]|%[^|]|%[^|]|%d|%lf|%d|%[^\n]\n",
		item.orderId, item.username, item.dishId, item.dishName,
		&item.quantity, &item.totalPrice, &item.status, item.orderTime) == 8) {
...
void saveCommentsToFile(void) {
	...
	while (curr) {
		fprintf(fp, "%s|%s|%s|%s|%d|%s|%s\n",
			curr->data.commentId, curr->data.orderId, curr->data.dishId,
			curr->data.username, curr->data.score, curr->data.content, curr->data.commentTime);
		curr = curr->next;
	}
	fclose(fp);
}
void loadCommentsFromFile(void) {
	...
	if (!fp) return;
	Comment item;
	// 依次解析直到最后一个字段时间 (遇到换行符结束)
	while (fscanf(fp, "%[^|]|%[^|]|%[^|]|%[^|]|%d|%[^|]|%[^\n]\n",
		item.commentId, item.orderId, item.dishId, item.username,
		&item.score, item.content, item.commentTime) == 7) {
...
// --- 恢复默认顺序 (本质是按菜品 ID 升序排列) ---
void sortDishesByIdAsc() {
	if (!gDishHead || !gDishHead->next) return;
	for (DishNode* i = gDishHead; i != NULL; i = i->next) {
		for (DishNode* j = i->next; j != NULL; j = j->next) {
			// strcmp > 0 表示 i 的 ID 字典序大于 j 的 ID
			if (strcmp(i->data.dishId, j->data.dishId) > 0) {
				Dish temp = i->data;
				i->data = j->data;
				j->data = temp;
			}
...
void drawStudentBrowsePage(void) {
	...
	// 【修改点】如果有搜索条件，或者 gIsSorted 为 1（被排序过），就显示“恢复默认”按钮
	if (gSearchKeyword[0] != '\0' || gComboSearchWindow[0] != '\0' || gComboSearchCategory[0] != '\0' || gIsSorted == 1) {
...
void handleStudentBrowsePage(const MOUSEMSG* msg) {
	if (msg->uMsg == WM_MOUSEMOVE) {
		...
		// 修改为：
		if ((gSearchKeyword[0] != '\0' || gComboSearchWindow[0] != '\0' || gComboSearchCategory[0] != '\0' || gIsSorted == 1) && pointInButton(msg->x, msg->y, &gBtnClearSearch)) gHoverIndex = 111;
		...
		// --- 清除条件并恢复默认顺序 ---
		else if (pointInButton(msg->x, msg->y, &gBtnClearSearch)) {
			gSearchKeyword[0] = '\0';
			gComboSearchWindow[0] = '\0';
			gComboSearchCategory[0] = '\0';
			gTableCurrentPage = 1;

			// 调用此函数，将物理链表重新洗牌，恢复到按 ID 排序的初始状态
			sortDishesByIdAsc();
			showGuiInfo("已恢复", "搜索条件已清空，列表已恢复默认顺序！");
		}
		...
		// --- 清除条件 ---
		// --- 恢复默认 / 清除条件 ---
		// 【修改点】判断条件同样加上 gIsSorted == 1
		else if ((gSearchKeyword[0] != '\0' || gComboSearchWindow[0] != '\0' || gComboSearchCategory[0] != '\0' || gIsSorted == 1) && pointInButton(msg->x, msg->y, &gBtnClearSearch)) {
			...
			// 2. 将物理链表重新洗牌，恢复到按 ID 排序的初始状态
			sortDishesByIdAsc(); // （这里调用我们上一轮写的那个按 ID 升序的函数）

			// 3. 状态重置
			gIsSorted = 0;       // 【新增】恢复为未排序状态
			gTableCurrentPage = 1;

			showGuiInfo("已恢复", "列表已恢复默认顺序！");
		}
